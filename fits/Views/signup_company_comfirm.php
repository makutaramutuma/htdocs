...
<main class="form-signin">
  ...
  <form action="signup_company_complete.php" method="POST">
    <img class="mb-4"  src="img/signup.png" alt="" width="120" height="120">
    <h1 class="h3 mb-4 fw-normal">登録情報確認画面</h1>
    <div class="">

    <p class="mt-3 mb-1">企業名</p>
    <div class="input-group mb-3">
   <p class="fw-bold"><?=htmlspecialchars($_POST['name']) ?></p>

</div>

<p class="mt-3 mb-1">メールアドレス</p>
<div class="input-group mb-3">
<p class="fw-bold"><?=htmlspecialchars($_POST['mail']) ?></p>

</div>

<p class="mt-3 mb-1">パスワード</p>
<div class="input-group mb-3">
<p class="fw-bold"><?=htmlspecialchars($_POST['password']) ?></p>
...
<p class="fw-bold" style="white-space:pre-wrap;"><?=htmlspecialchars($_POST['profile']) ?></p>

</div>
<input type="hidden" name="name" value="<?=htmlspecialchars($_POST['name']) ?>">
<input type="hidden" name="image" value="<?=('img/'.$_POST['image']) ?>">
<input type="hidden" name="mail" value="<?=htmlspecialchars($_POST['mail']) ?>">
...
<button type="button" class="btn btn-secondary" onclick="location.href='./signup_company.php'">戻る</button>
